...
def simulate_nfa_to_dfa():
    try:
        ...
        for trans in nfa_transitions:
            ...
            nfa_transition_dict.setdefault((src, sym), set()).add(dest)
        ...
        while queue:
            ...
            for sym in alphabet:
                ...
                for state in current_set:
                    next_set.update(nfa_transition_dict.get((state, sym), set()))
        ...
        # Simulate DFA on the input string to check acceptance
        current_state = state_map[frozenset([start_state])]
        for symbol in input_string:
            transition_found = False
            for transition in dfa_transitions:
                src, sym, dest = transition.split(',')
                if src == current_state and sym == symbol:
                    current_state = dest
                    transition_found = True
                    break

            if not transition_found:  # No valid transition found for the symbol
                result_label.config(text="Input string rejected", fg="red")
                return  # Exit the function as the string is rejected

        # After processing the string, check if the final state is an accepting state
        if current_state in dfa_accept_states:
            result_label.config(text="Input string accepted", fg="green")
        else:
            result_label.config(text="Input string rejected", fg="red")
...
simulate_button = tk.Button(input_frame, text="Simulate NFA to DFA", command=simulate_nfa_to_dfa, bg="white", fg="black")
